  ...
  try {
    if (!process.env.OPENAI_API_KEY) {
      return res.status(500).json({
        error: 'OPENAI_API_KEY is missing. Add it to a .env file before running the app.',
      });
    }

    const pastedEmail = typeof req.body.email === 'string' ? req.body.email : '';
    ...
    const message =
      error instanceof Error ? error.message : 'Something went wrong while summarizing the email.';
